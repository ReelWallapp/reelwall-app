...
function mergeCatchesByIdOrImage(
  supabaseCatches: CatchItem[],
  localCatches: CatchItem[]
) {
  const merged: CatchItem[] = [];
  const seen = new Set<string>();

  for (const item of [...supabaseCatches, ...localCatches]) {
...
export default function CollectionsScreen() {
  ...
  const [showCollectionPhotosModal, setShowCollectionPhotosModal] =
    useState(false);
...
const collectionShareCardRef = useRef<View | null>(null);
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'collection_catches' },
        () => {
          loadData();
        }
      )
  ...
  async function loadData() {
    try {
      ...
      const typedCollections =
        ((collectionRows || []) as Omit<CollectionItem, 'catchIds'>[]).map(
          (item) => ({
            ...item,
            catchIds: [],
          })
  ...
  async function deleteCollection(collectionId: string) {
    Alert.alert(
      'Delete collection?',
      'This removes the collection, but not the catches inside it.',
      [
  ...
  async function updateCollectionCoverFromCatch(
    collectionId: string,
  ...
  async function removeCatchFromCollection(collectionId: string, catchId: string) {
    Alert.alert(
      ...
      'This catch will stay on your wall.',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Remove',
...
>
        {coverImage ? (
          ...
          <View style={styles.heroFallback}>
            <Text style={styles.heroFallbackText}>Tap Manage Catches to Add</Text>
          </View>
        )}
          ...
          !loading ? (
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptyStateText}>
                Create your first collection and start grouping your catches.
              </Text>
            </View>
          ) : null
        }
          ...
          >
            <View style={styles.modalCard}>
              ...
              <TextInput
                ...
                placeholder="Summer kings with the crew"
                placeholderTextColor="#7D8FA3"
              ...
              <View style={styles.publicRow}>
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.publicHelp}>
                    Public collections can be shown on your web wall.
                  </Text>
                </View>

                <Switch
                  value={draftIsPublic}
                  onValueChange={setDraftIsPublic}
                  trackColor={{ false: '#29435B', true: PRIMARY }}
                  thumbColor="#F5F7FA"
                />
              </View>
            ...
            <View style={styles.manageModalCard}>
              <View style={styles.modalHeaderRow}>
                <View style={{ flex: 1, paddingRight: 12 }}>
                  <Text style={styles.modalTitleSmall}>Manage Catches</Text>
              ...
              {filteredManageCatches.length === 0 ? (
                <View style={styles.emptyInlineState}>
                  <Text style={styles.emptyInlineTitle}>No matches</Text>
          ...
          <View style={styles.galleryModalCard}>
            ...
            {galleryPhotos.length ? (
              ...
              <View style={styles.emptyInlineState}>
                ...
                <Text style={styles.emptyInlineText}>
                  This collection does not have any photos yet.
                </Text>
              </View>
            )}
